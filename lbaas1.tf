...
#AD1
resource "oci_core_subnet" "subnet1_1" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[0],"name")}"
  cidr_block          = "10.1.20.0/24"
  display_name        = "subnet1_1"
  dns_label           = "subnet1_1"
...
#AD2
resource "oci_core_subnet" "subnet1_2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[1],"name")}"
  cidr_block          = "10.1.21.0/24"
  display_name        = "subnet1_2"
  dns_label           = "subnet1_2"
  security_list_ids   = ["${oci_core_default_security_list.default-security-list.id}"]
  compartment_id      = "${var.compartment_ocid}"
  vcn_id              = "${oci_core_virtual_network.vcn1.id}"
...
resource "oci_core_instance" "instance1_1" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[0],"name")}"
  compartment_id      = "${var.compartment_ocid}"
  display_name        = "be-instance1_1"
  image               = "${var.InstanceImageOCID[var.region]}"
  shape               = "VM.Standard2.1"
  subnet_id           = "${oci_core_subnet.subnet1_1.id}"
  hostname_label      = "be-instance1_1"
...
resource "oci_core_instance" "instance1_2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[1],"name")}"
  compartment_id      = "${var.compartment_ocid}"
  display_name        = "be-instance1_2"
  image               = "${var.InstanceImageOCID[var.region]}"
  shape               = "VM.Standard2.1"
  subnet_id           = "${oci_core_subnet.subnet1_2.id}"
  hostname_label      = "be-instance1_2"
...
resource "oci_load_balancer" "lb1" {
  ...
  subnet_ids = [
    "${oci_core_subnet.subnet1_1.id}",
    "${oci_core_subnet.subnet1_2.id}",
  ]
...
resource "oci_load_balancer_listener" "lb-listener1_1" {
  load_balancer_id         = "${oci_load_balancer.lb1.id}"
  name                     = "http"
...
resource "oci_load_balancer_listener" "lb-listener1_2" {
  load_balancer_id         = "${oci_load_balancer.lb1.id}"
  name                     = "https"
  default_backend_set_name = "${oci_load_balancer_backendset.lb-bes1.id}"
  port                     = 443
...
resource "oci_load_balancer_backend" "lb-be1_1" {
  load_balancer_id = "${oci_load_balancer.lb1.id}"
  backendset_name  = "${oci_load_balancer_backendset.lb-bes1.id}"
  ip_address       = "${oci_core_instance.instance1_1.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
...
resource "oci_load_balancer_backend" "lb-be1_2" {
  load_balancer_id = "${oci_load_balancer.lb1.id}"
  backendset_name  = "${oci_load_balancer_backendset.lb-bes1.id}"
  ip_address       = "${oci_core_instance.instance1_2.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
