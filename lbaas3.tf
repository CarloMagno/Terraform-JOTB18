...
#AD1
resource "oci_core_subnet" "subnet3_1" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[0],"name")}"
  cidr_block          = "10.1.20.0/24"
  display_name        = "subnet3_1"
  dns_label           = "subnet3_1"
  security_list_ids   = ["${oci_core_default_security_list.default-security-list.id}"]
  compartment_id      = "${var.compartment_ocid}"
  vcn_id              = "${oci_core_virtual_network.vcn3.id}"
  route_table_id      = "${oci_core_route_table.routetable3.id}"
  dhcp_options_id     = "${oci_core_virtual_network.vcn3.default_dhcp_options_id}"
...
#AD2
resource "oci_core_subnet" "subnet3_2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[2],"name")}"
  cidr_block          = "10.1.21.0/24"
  display_name        = "subnet3_2"
  dns_label           = "subnet3_2"
  security_list_ids   = ["${oci_core_default_security_list.default-security-list.id}"]
  compartment_id      = "${var.compartment_ocid}"
  vcn_id              = "${oci_core_virtual_network.vcn3.id}"
...
resource "oci_core_internet_gateway" "internetgateway3" {
  compartment_id = "${var.compartment_ocid}"
  display_name   = "internetgateway3"
  vcn_id         = "${oci_core_virtual_network.vcn3.id}"
}

resource "oci_core_route_table" "routetable3" {
  compartment_id = "${var.compartment_ocid}"
  vcn_id         = "${oci_core_virtual_network.vcn3.id}"
  display_name   = "routetable3"

  route_rules {
    cidr_block        = "0.0.0.0/0"
    network_entity_id = "${oci_core_internet_gateway.internetgateway3.id}"
  }
}

resource "oci_core_default_security_list" "default-security-list" {
  manage_default_resource_id = "${oci_core_virtual_network.vcn3.default_security_list_id}"
...
resource "oci_core_instance" "instance3_1" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[0],"name")}"
  compartment_id      = "${var.compartment_ocid}"
  display_name        = "be-instance3_1"
  image               = "${var.InstanceImageOCID[var.region]}"
  shape               = "VM.Standard2.1"
  subnet_id           = "${oci_core_subnet.subnet3_1.id}"
  hostname_label      = "be-instance3_1"
...
resource "oci_core_instance" "instance3_2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[2],"name")}"
  compartment_id      = "${var.compartment_ocid}"
  display_name        = "be-instance3_2"
  image               = "${var.InstanceImageOCID[var.region]}"
  shape               = "VM.Standard2.1"
  subnet_id           = "${oci_core_subnet.subnet3_2.id}"
  hostname_label      = "be-instance3_2"
...
resource "oci_load_balancer" "lb3" {
  ...
  subnet_ids = [
    "${oci_core_subnet.subnet3_1.id}",
    "${oci_core_subnet.subnet3_2.id}",
  ]

  display_name = "lb3"
}

resource "oci_load_balancer_backendset" "lb-bes3" {
  name             = "lb-bes3"
...
resource "oci_load_balancer_listener" "lb-listener3_1" {
  load_balancer_id         = "${oci_load_balancer.lb3.id}"
  name                     = "http"
  default_backend_set_name = "${oci_load_balancer_backendset.lb-bes3.id}"
  port                     = 80
  protocol                 = "HTTP"
}

resource "oci_load_balancer_listener" "lb-listener3_2" {
  load_balancer_id         = "${oci_load_balancer.lb3.id}"
  name                     = "https"
  default_backend_set_name = "${oci_load_balancer_backendset.lb-bes3.id}"
  port                     = 443
  ...
  ssl_configuration {
    certificate_name        = "${oci_load_balancer_certificate.lb-cert3.certificate_name}"
    verify_peer_certificate = false
  }
}

resource "oci_load_balancer_backend" "lb-be3_1" {
  load_balancer_id = "${oci_load_balancer.lb3.id}"
  backendset_name  = "${oci_load_balancer_backendset.lb-bes3.id}"
  ip_address       = "${oci_core_instance.instance3_1.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
...
resource "oci_load_balancer_backend" "lb-be3_2" {
  load_balancer_id = "${oci_load_balancer.lb3.id}"
  backendset_name  = "${oci_load_balancer_backendset.lb-bes3.id}"
  ip_address       = "${oci_core_instance.instance3_2.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
...
output "lb_public_ip" {
  value = ["${oci_load_balancer.lb3.ip_addresses}"]
}
